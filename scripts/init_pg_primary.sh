...
log "Modifying pg_hba.conf to enforce SSL and allow replication for repmgr..."


# Allow replication connections from any host in the Docker subnet using SSL
echo "hostssl replication repmgr $DOCKER_SUBNET md5" >> "$PG_HBA_CONF"

# Allow replication connections from any host in the Docker subnet without SSL (optional, not recommended in production)
echo "host replication repmgr $DOCKER_SUBNET trust" >> "$PG_HBA_CONF"


# Allow non-encrypted connections for Vault
echo "host all $VAULT_USER $DOCKER_SUBNET trust" >> "$PG_HBA_CONF"
...
echo "hostssl all all $POSTGRES_DB md5" >> "$PG_HBA_CONF"
...
echo "host all all $VAULT_CONTAINER_NAME trust" >> "$PG_HBA_CONF"
echo "hostssl all all $VAULT_CONTAINER_NAME md5" >> "$PG_HBA_CONF"
...
log "Creating repmgr user for replication..."
...
# Create repmgr user if it doesn't exist
psql -U postgres -d postgres -c "DO \$\$ BEGIN IF NOT EXISTS (SELECT FROM pg_catalog.pg_user WHERE usename = '$REPMGR_USER') THEN CREATE ROLE $REPMGR_USER WITH LOGIN REPLICATION PASSWORD '$REPMGR_PASSWORD'; END IF; END \$\$;" || error_exit "Failed to create repmgr user."
# Create repmgr database if it doesn't exist
psql -U postgres -tc "SELECT 1 FROM pg_database WHERE datname='repmgr'" | grep -q 1 || psql -U postgres -c "CREATE DATABASE repmgr OWNER repmgr;"
# Grant privileges on repmgr database
psql -U postgres -c "GRANT ALL PRIVILEGES ON DATABASE repmgr TO repmgr;"

echo "repmgr user and database setup completed."
log "Creating user '$POSTGRES_USER' if it does not exist..."
psql -U postgres -d postgres -c "DO \$\$ BEGIN IF NOT EXISTS (SELECT FROM pg_catalog.pg_user WHERE usename = '$POSTGRES_USER') THEN CREATE ROLE $POSTGRES_USER WITH LOGIN PASSWORD '$POSTGRES_PASSWORD'; END IF; END \$\$;" || error_exit "Failed to create user '$POSTGRES_USER'."
